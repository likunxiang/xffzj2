<template>
  <div>
    <el-dialog title="详情" width="900px" :visible.sync="isOpen" destroy-on-close @close="beforeClose">
      ...
      <addHRmessage v-if="isMessage" @close="closeAddMessage"></addHRmessage>
      <guideResult v-if="isGuide" @close="closeGuide" :row="openRow"></guideResult>
      <purchaseResult v-if="isPurchase" @close="closePurchase"></purchaseResult>
      <supplyResult v-if="isSupply" @close="closeSupply"></supplyResult>
      <userMessage v-if="isUserMessage" @close="closePersonalinformation"></userMessage>
    </el-dialog>
  </div>
</template>
...
<script>
  ...
  import { getTakeBackUserList } from '@/api/operationRoleApi/dispatchingManagementApi.js'
